...
var _ = Describe("func ExtractMIC()", func() {
	It("returns the MIC from the end of the ciphertext", func() {
		ciphertext := []byte{
			0x5a, 0x98, 0x9a, 0xe4, 0x2e, 0x8d, // payload
			0x84, 0x7f, 0x53, 0x5c, 0x30, 0x07, 0xe6, 0x15, // mic
			0x0c, 0xd6, 0x58, 0x67, 0xf2, 0xb8, 0x17, 0xdb,
		}

		Expect(ExtractMIC(ciphertext)).To(Equal(MIC{
			0x84, 0x7f, 0x53, 0x5c, 0x30, 0x07, 0xe6, 0x15,
			0x0c, 0xd6, 0x58, 0x67, 0xf2, 0xb8, 0x17, 0xdb,
		}))
	})
})
